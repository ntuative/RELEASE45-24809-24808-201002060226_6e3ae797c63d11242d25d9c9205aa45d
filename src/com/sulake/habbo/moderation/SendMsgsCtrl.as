package com.sulake.habbo.moderation
{
   import com.sulake.core.runtime.IDisposable;
   import com.sulake.core.window.IWindow;
   import com.sulake.core.window.components.IDropMenuWindow;
   import com.sulake.core.window.components.IFrameWindow;
   import com.sulake.core.window.components.ITextFieldWindow;
   import com.sulake.core.window.events.WindowEvent;
   import com.sulake.core.window.events.WindowMouseEvent;
   import com.sulake.habbo.communication.messages.outgoing.moderator.ModMessageMessageComposer;
   import com.sulake.habbo.window.utils.IAlertDialog;
   import flash.events.Event;
   
   public class SendMsgsCtrl implements IDisposable, TrackedWindow
   {
       
      
      private var var_1119:String;
      
      private var _disposed:Boolean;
      
      private var var_1403:String;
      
      private var var_1120:int;
      
      private var var_41:ModerationManager;
      
      private var var_734:IDropMenuWindow;
      
      private var var_362:ITextFieldWindow;
      
      private var var_45:IFrameWindow;
      
      private var var_297:Boolean = true;
      
      public function SendMsgsCtrl(param1:ModerationManager, param2:int, param3:String, param4:String)
      {
         super();
         var_41 = param1;
         var_1120 = param2;
         var_1403 = param3;
         var_1119 = param4;
      }
      
      private function onInputClick(param1:WindowEvent, param2:IWindow) : void
      {
         if(param1.type != WindowEvent.const_181)
         {
            return;
         }
         if(!var_297)
         {
            return;
         }
         var_362.text = "";
         var_297 = false;
      }
      
      public function getFrame() : IFrameWindow
      {
         return var_45;
      }
      
      public function getId() : String
      {
         return var_1403;
      }
      
      private function onSendMessageButton(param1:WindowEvent, param2:IWindow) : void
      {
         if(param1.type != WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            return;
         }
         Logger.log("Sending message...");
         if(var_297 || false)
         {
            var_41.windowManager.alert("Alert","You must input a message to the user",0,onAlertClose);
            return;
         }
         var_41.connection.send(new ModMessageMessageComposer(var_1120,var_362.text,var_1119));
         this.dispose();
      }
      
      public function show() : void
      {
         var_45 = IFrameWindow(var_41.getXmlWindow("send_msgs"));
         var_45.caption = "Msg To: " + var_1403;
         var_45.findChildByName("send_message_but").procedure = onSendMessageButton;
         var_362 = ITextFieldWindow(var_45.findChildByName("message_input"));
         var_362.procedure = onInputClick;
         var_734 = IDropMenuWindow(var_45.findChildByName("msgTemplatesSelect"));
         prepareMsgSelect(var_734);
         var_734.procedure = onSelectTemplate;
         var _loc1_:IWindow = var_45.findChildByTag("close");
         _loc1_.procedure = onClose;
         var_45.visible = true;
      }
      
      private function prepareMsgSelect(param1:IDropMenuWindow) : void
      {
         Logger.log("MSG TEMPLATES: " + var_41.initMsg.messageTemplates.length);
         param1.populate(var_41.initMsg.messageTemplates);
      }
      
      public function dispose() : void
      {
         if(_disposed)
         {
            return;
         }
         _disposed = true;
         if(var_45 != null)
         {
            var_45.destroy();
            var_45 = null;
         }
         var_734 = null;
         var_362 = null;
         var_41 = null;
      }
      
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         if(param1.type != WindowEvent.const_35)
         {
            return;
         }
         var _loc3_:String = var_41.initMsg.messageTemplates["null"];
         if(_loc3_ != null)
         {
            var_297 = false;
            var_362.text = _loc3_;
         }
      }
      
      public function get disposed() : Boolean
      {
         return _disposed;
      }
      
      private function onClose(param1:WindowEvent, param2:IWindow) : void
      {
         if(param1.type != WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            return;
         }
         dispose();
      }
      
      private function onAlertClose(param1:IAlertDialog, param2:Event) : void
      {
         param1.dispose();
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1365;
      }
   }
}
