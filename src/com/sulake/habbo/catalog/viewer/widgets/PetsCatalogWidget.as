package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_407 >= var_294.length)
         {
            return "";
         }
         var petColor:IPetColor = var_294[var_407] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_581[var_1531] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_407 = param1.index;
         if(var_407 < 0 || var_407 > var_294.length)
         {
            var_407 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_518.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         var_865 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_294 = [];
         var_581 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_865);
               if(_loc11_)
               {
                  var_581 = _loc11_.breeds;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_581.length)
         {
            _loc12_ = var_581[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_865,_loc12_.id),getRaceLocalizationKey(var_865,_loc12_.id)));
            _loc6_++;
         }
         var_518.add(var_865,_loc5_);
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_518.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_407 >= 0 && var_407 < var_294.length)
         {
            _loc7_ = var_294[var_407] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_581[var_1531] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_865,_loc3_.id,_loc2_,AvatarScaleType.const_33);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_288,AvatarAction.const_620);
